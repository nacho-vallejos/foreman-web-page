...
function revealOnScroll(){
  const items = Array.from(d.querySelectorAll("section, [data-iv-reveal]"));
  items.forEach(el=>el.classList.add("iv-reveal"));
  if("IntersectionObserver" in w){
    const io = new IntersectionObserver((entries)=>{
      entries.forEach(en=>{
        if(en.isIntersecting){ en.target.classList.add("iv-visible"); io.unobserve(en.target); }
      });
    },{threshold:.16});
...
function autoTargetHero(){
  // Intenta detectar el título principal y subtítulo existentes sin cambiar mi HTML.
  const h1 = d.querySelector("[data-iv-hero] h1, main h1, header ~ main h1, h1");
  if(h1) splitify(h1);
  // Subtítulo = primer <p> o elemento inmediatamente siguiente de tipo bloque
  let sub = h1 ? h1.nextElementSibling : null;
  if(sub && (sub.tagName==="P" || sub.tagName==="H2" || sub.dataset?.subtitle)){
    splitify(sub);
  }
}
